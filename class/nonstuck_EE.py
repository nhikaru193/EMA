...
# --- ミッションステージのパラメータ ---
# 放出判定ステージ (RoverReleaseDetectorのデフォルト設定を使用)
...
# 着地安定性判定ステージ (RoverLandingDetectorのデフォルト設定を使用)
...
if __name__ == "__main__":
    ...
    try: # この try は if __name__ == "__main__": の直下で一段インデントされているべき
        ...
        ejection_detector = RoverReleaseDetector(
            # RoverReleaseDetectorの__init__を修正し、bno_sensorとi2c_bus_instanceを引数で受け取るようにする
