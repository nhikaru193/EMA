...
class FlagDetector:
    ...
    def detect(self):
        ...
        upper_black = np.array([180, 255, 35]) # この値は環境に応じて調整
            ...
            for cnt in contours_in_roi:
                ...
                if shape_name != "不明" and approx is not None:
                    ...
                    if M_shape["m00"] != 0:
                        ...
                        # ★★★ 変更点: 面積を計算して辞書に追加 ★★★
                        shape_area = cv2.contourArea(approx_global)
    ...
    try:
        ...
        if detected_data:
            ...
            for i, flag in enumerate(detected_data):
                # 検出された図形の名前と面積のリストを作成
                shape_details = [f"{s['name']} (Area: {s['area']:.0f})" for s in flag["shapes"]]
                print(f"フラッグ {i+1}: 位置={flag['location']}, 図形={', '.join(shape_details)}")
        else:
