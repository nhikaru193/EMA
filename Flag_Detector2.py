...
class FlagDetector:
    ...
    def detect(self):
        ...
        upper_black = np.array([180, 255, 25]) # この値は環境に応じて調整　60とかにしたら明度上がって、背景の森も読み取ってしまう
        ...
        for region_contour in valid_black_regions:
            ...
            _, binary_roi = cv2.threshold(gray_roi, 50, 255, cv2.THRESH_BINARY) #80にすると、グレーを読み取る。一方で120などにすると、黒領域の白飛び部分を検出するとがある
